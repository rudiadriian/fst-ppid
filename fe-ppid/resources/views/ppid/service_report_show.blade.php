...
@section('title', $data['title'] . ' | ' . __('Laporan Pelayanan Informasi'))
    ...
    <section class="relative fs-gradient overflow-hidden">
        ...
        <div class="relative z-10 max-w-screen-2xl mx-auto px-6 lg:px-8 py-14 lg:py-16">
            ...
            <div class="flex flex-wrap items-center gap-3 mb-4">
                <span class="px-3 py-1 inline-flex text-xs font-semibold rounded-full bg-white/15 text-white">{{ __('Tahun') }} {{ $data['year'] }}</span>
                <span class="px-3 py-1 inline-flex text-xs font-semibold rounded-full bg-white/15 text-white">{{ __($data['periode']) }}</span>
        ...
        <div class="max-w-screen-2xl mx-auto px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-[#0B2A1D] rounded-2xl border border-gray-100 dark:border-white/10 p-4 sm:p-6">
                <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
                    ...
                    <span class="text-[11px] font-bold uppercase tracking-[0.14em] text-gray-500 dark:text-gray-400">{{ __($data['periode']) }} {{ $data['year'] }}</span>
                </div>
            ...
            @if (!empty($lainnya))
                <div class="mt-14">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{!! $judulDua(__('Laporan Lainnya'), 1) !!}</h2>
                            ...
                            <article class="group relative flex flex-col bg-white dark:bg-[#0B2A1D] rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden">
                                ...
                                <div class="flex-1 p-5">
                                    <div class="flex flex-wrap items-center gap-2 mb-2">
                                        <span class="px-2.5 py-0.5 inline-flex text-[11px] font-semibold rounded-full bg-emerald-50 text-[#10462F] dark:bg-white/5 dark:text-[#3E9C6C]">
                                            {{ __('Tahun') }} {{ $lain['year'] }}
                                        </span>
