            ...
            @if (!empty($data['years']))
                <form method="GET" action="{{ route('ppid.register') }}" class="mb-6 flex flex-wrap items-center gap-3">
                    ...
                    <select id="tahun" name="tahun"
                            class="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-normal text-gray-700 focus:border-[#008060] focus:ring-[#008060] dark:border-white/10 dark:bg-[#121a17] dark:text-gray-200">
                        <option value="">{{ __('Semua Tahun') }}</option>
                        @foreach ($data['years'] as $year)
                            <option value="{{ $year }}" @selected($data['selected_year'] == $year)>{{ $year }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="rounded-xl bg-[#008060] px-5 py-2.5 text-sm font-semibold text-white transition-colors duration-200 hover:bg-[#00664e]">{{ __('Tampilkan') }}</button>
                </form>
            @endif
                                ...
                                <tr class="transition-colors duration-150 hover:bg-[#F8FAFC] dark:hover:bg-white/5">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold
                                            @if (in_array($item['status'], ['selesai', 'disetujui'])) bg-emerald-50 text-[#008060]
            ...
            <div class="mt-8 flex items-start gap-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-6 sm:p-8 dark:border-emerald-500/20 dark:bg-emerald-500/10">
                ...
                <div>
                    ...
                    <p class="text-base font-normal leading-relaxed text-gray-600 dark:text-gray-300">
                        ...
                        <a href="{{ route('ppid.status') }}" class="font-semibold text-[#008060] hover:text-[#00664e] dark:text-[#00A66C]">{{ __('Cek Status Tiket') }}</a>.
                    </p>
                </div>
            </div>
