...
@section('content')
    ...
    <section class="relative fs-gradient overflow-hidden">
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-20 text-center">
            ...
            <h1 class="text-4xl lg:text-5xl font-bold text-white leading-tight">{!! $judulDua(__('Formulir Permohonan Informasi Publik'), 2, 'fs-title-accent-soft') !!}</h1>
            ...
            class="bg-white dark:bg-[#0B2A1D] p-6 sm:p-10 rounded-2xl shadow-sm border border-gray-100 dark:border-white/10">
                ...
                <div x-show="isSuccess" x-transition.opacity class="text-center py-4" role="alert">
                    <div class="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-5">
                        <svg class="w-10 h-10 text-[#10462F]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    ...
                    <button @click="isSuccess = false; window.scrollTo({ top: 0, behavior: 'smooth' })" class="mt-6 text-sm font-semibold text-[#10462F] hover:text-[#0B3524]">
                ...
                <form @submit.prevent="submitForm()" x-show="!isSuccess" x-transition>
                    ...
                    @php
                        ...
                        $radioCardClass = 'flex items-center p-3.5 bg-gray-50 rounded-xl border border-gray-200 dark:border-white/10 hover:border-[#10462F] has-[:checked]:border-[#10462F] has-[:checked]:bg-emerald-50/50 transition cursor-pointer';
                    @endphp

                    {{-- Data Pemohon --}}
                    <div class="p-6 bg-[#F3ECDD] dark:bg-[#082217] rounded-2xl border border-gray-100 dark:border-white/10 mb-6">
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2.5">
                            <span class="w-9 h-9 bg-emerald-50 text-[#10462F] rounded-xl flex items-center justify-center">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                            </span>
                            {!! $judulDua(__('Data Pemohon')) !!}
                        </h2>
                    ...
                    {{-- Cara Mendapatkan --}}
                    <div class="p-6 bg-[#F3ECDD] dark:bg-[#082217] rounded-2xl border border-gray-100 dark:border-white/10">
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2.5">
                            <span class="w-9 h-9 bg-emerald-50 text-[#10462F] rounded-xl flex items-center justify-center">
                            ...
                            {!! $judulDua(__("Cara Mendapatkan Informasi")) !!}
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <div class="space-y-3">
                                    <label class="{{ $radioCardClass }}">
                                        <input type="radio" x-model="form.format" value="softcopy" class="text-[#10462F] focus:ring-[#10462F]">
                                        <span class="ml-3 text-gray-700 dark:text-gray-300 font-medium">{{ __('Softcopy (Digital)') }}</span>
                                    </label>
                                    <label class="{{ $radioCardClass }}">
                                        <input type="radio" x-model="form.format" value="hardcopy" class="text-[#10462F] focus:ring-[#10462F]">
                            ...
                            <div>
                                ...
                                <div class="space-y-3">
                                    <label class="{{ $radioCardClass }}">
                                        <input type="radio" x-model="form.way_to_get" value="email" class="text-[#10462F] focus:ring-[#10462F]">
                                        <span class="ml-3 text-gray-700 dark:text-gray-300 font-medium">{{ __('Melalui Email') }}</span>
                                    </label>
                                    <label class="{{ $radioCardClass }}">
                                        <input type="radio" x-model="form.way_to_get" value="ambil_langsung" class="text-[#10462F] focus:ring-[#10462F]">
                                        <span class="ml-3 text-gray-700 dark:text-gray-300 font-medium">{{ __('Diambil Langsung di Kantor') }}</span>
                                    </label>
                                    <label class="{{ $radioCardClass }}">
                                        <input type="radio" x-model="form.way_to_get" value="pos" class="text-[#10462F] focus:ring-[#10462F]">
                    ...
                    <div class="mt-8 flex justify-end">
                        <button type="submit" :disabled="isSubmitting"
                            class="inline-flex items-center justify-center py-3.5 px-10 fs-gradient-accent text-white text-base font-semibold rounded-xl shadow-lg shadow-emerald-900/20 hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed">
