...
@section('title', __('Hasil Pencarian') . ' | ' . __('PPID FSTJ'))
@section('content')
    ...
    <section class="relative fs-gradient overflow-hidden">
        ...
        <div class="relative z-10 max-w-screen-2xl mx-auto px-6 lg:px-8 py-16 lg:py-20 text-center">
            ...
            <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-white leading-tight">{!! $judulDua(__('Hasil Pencarian'), 1, 'fs-title-accent-soft') !!}</h1>
            @if ($query !== '')
            ...
            <form action="{{ route('search') }}" method="GET" class="mx-auto mt-8 flex max-w-xl gap-2">
                ...
                <button type="submit" class="rounded-xl bg-white px-6 py-3 text-sm font-semibold text-[#10462F] transition-colors hover:bg-emerald-50">{{ __('Cari') }}</button>
            </form>
        </div>
    </section>
        ...
        <div class="max-w-6xl mx-auto px-6 lg:px-8">
            ...
            @else
                @forelse ($results as $result)
                    <a href="{{ $result['url'] }}"
                       class="mb-4 flex items-start justify-between gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:border-[#10462F] dark:border-white/10 dark:bg-[#0B2A1D]">
                        <span>
                        ...
                        <svg class="mt-1 h-5 w-5 flex-shrink-0 text-[#10462F]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5l7 7-7 7"/></svg>
                    </a>
                @empty
                    <div class="rounded-2xl border border-gray-100 bg-white px-6 py-16 text-center shadow-sm dark:border-white/10 dark:bg-[#0B2A1D]">
