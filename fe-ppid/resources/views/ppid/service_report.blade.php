...
@section('content')
    ...
    {{-- KONTEN — daftar kartu: sampul dokumen di kiri, keterangan di kanan.
         Bentuknya sengaja dibuat sama dengan modul Regulasi. --}}
            ...
            <div x-data="{ cari: '' }">
                <div class="flex flex-col md:flex-row justify-between md:items-center mb-8 gap-4">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                        {!! $judulDua(__('DAFTAR LAPORAN'), 1) !!}
                        <span class="ml-2 align-middle text-sm font-semibold text-gray-500 dark:text-gray-400">{{ count($data['reports']) }} {{ __('Dokumen') }}</span>
                    </h2>
                    ...
                    <div class="w-full md:w-auto md:min-w-[360px] relative">
                        ...
                        <input x-model="cari" type="search" placeholder="{{ __('Cari judul laporan...') }}"
                               aria-label="{{ __('Cari judul laporan') }}"
                ...
                <div class="space-y-5">
                    @foreach ($data['reports'] as $laporan)
                        <article x-show="cari === '' || '{{ \Illuminate\Support\Str::lower(addslashes($laporan['title'].' '.$laporan['year'])) }}'.includes(cari.toLowerCase())"
                            ...
                            <div class="flex flex-col sm:flex-row">
                                ...
                                {{-- Sampul dokumen: halaman pertama berkas yang
                                     diunggah, digambar ke <canvas> lewat pdf.js
                                     (resources/js/sampul-pdf.js). Selama belum
                                     tergambar — atau bila berkasnya belum ada —
                                ...
                                <div class="sm:w-56 lg:w-64 flex-shrink-0 bg-white dark:bg-[#0B2A1D] border-b sm:border-b-0 sm:border-r border-gray-100 dark:border-white/10 flex items-center justify-center p-4">
                                    <div class="relative w-full h-56 sm:h-64 overflow-hidden rounded-xl border border-gray-100 dark:border-white/10 bg-white"
                                         @if (!empty($laporan['link']) && $laporan['ext'] === 'pdf') data-pdf-cover="{{ $laporan['link'] }}" @endif
                                         role="img"
                                         aria-label="{{ __('Halaman pertama dokumen') }}: {{ $laporan['title'] }}">
                                        @if (!empty($laporan['link']) && in_array($laporan['ext'], ['jpg', 'jpeg', 'png', 'webp'], true))
                                            <img src="{{ $laporan['link'] }}" alt="{{ $laporan['title'] }}" loading="lazy"
                                ...
                                <div class="flex-1 p-6 sm:p-7">
                                    <div class="flex flex-wrap items-center gap-3 mb-3">
                                        <span class="px-3 py-1 inline-flex text-xs font-semibold rounded-full bg-emerald-50 text-[#10462F] dark:bg-white/5 dark:text-[#3E9C6C]">
                                            {{ __('Tahun') }} {{ $laporan['year'] }}
                                        </span>
                                        ...
                                        @if (!empty($laporan['published']))
                                            <span class="inline-flex items-center gap-1.5 text-sm font-normal text-gray-500 dark:text-gray-400">
                                                ...
                                                {{ \App\Support\Cms::tanggalWaktu($laporan['published']) }}
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex flex-wrap items-center justify-between gap-3">
                                        ...
                                        <span class="inline-flex items-center gap-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                                            ...
                                            {{ __('Diunggah oleh') }}
                                            <span class="font-semibold text-gray-700 dark:text-gray-200">{{ $laporan['pengunggah'] ?: __('Petugas PPID') }}</span>
                                        </span>
