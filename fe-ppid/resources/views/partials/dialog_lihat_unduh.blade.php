{{-- Dialog "melihat atau mengunduh" untuk entri yang salinannya tidak berupa
     berkas unggahan: galeri Laporan Tahunan di beranda dan Daftar Informasi
     Dikecualikan.

     Satu dialog untuk seluruh halaman, bukan satu per entri: isinya diganti
     oleh event `buka-dialog-lihat-unduh` yang dikirim tombol yang diklik.

     Berbeda dari `partials.informasi_dialog` milik Daftar Informasi Publik:
     "Mengunduh" di sini tidak menunjuk berkas mana pun, melainkan mengantar ke
     Permohonan Informasi — salinannya tetap dilepas lewat keputusan petugas
     PPID, bukan lewat tautan langsung. Halaman yang memuat partial ini wajib
     memuatnya tepat satu kali. --}}
<div x-data="{
        terbuka: false,
        judul: '',
        tautan: null,
     ...
     @buka-dialog-lihat-unduh.window="buka($event.detail)"
    ...
    <div x-show="terbuka" x-transition.opacity
        ...
        <div x-show="terbuka" x-transition
            ...
            <p class="text-sm font-normal text-gray-600 dark:text-gray-300 leading-relaxed mb-6">
                {{ __('Apakah Anda ingin melihat atau mengunduh dokumen ini?') }}
                {{ __('Melihat terbuka untuk siapa saja; mengunduh salinannya memerlukan Permohonan Informasi yang disetujui petugas PPID.') }}
            </p>

            <div class="flex flex-col gap-3">
                <template x-if="tautan">
