...
import { useUbahPassword } from '../api/useAkun';

/**
 * Tab **Ubah Password** pada halaman Akun Saya.
 *
 * Password lama tetap dituntut walau tokennya sudah sah — token bisa terbawa
 * perangkat yang ditinggal terbuka, dan menuntut password lama membuat layar
 * yang lupa dikunci tidak cukup untuk mengambil alih akun. Syarat password
 * barunya disamakan dengan saat akun dibuat administrator; penegakannya di API,
 * yang di sini hanya disebutkan supaya tidak ditebak-tebak.
 */
...
export function AkunPasswordPanel() {
	const { t } = useTranslation();
	const { enqueueSnackbar } = useSnackbar();
	const ubahPassword = useUbahPassword();
	...
	const siap = isian.password_lama !== '' && cocok && !ubahPassword.isPending;
	...
	async function kirim(event: React.FormEvent) {
		...
		try {
			await ubahPassword.mutateAsync(isian);
			...
			if (err instanceof PpidApiError) {
				...
				setGalat(perField);
				// Galat yang tidak menempel pada isian mana pun — mis. 429 dari rem
				// laju — tetap harus terbaca, bukan hilang tanpa jejak.
	...
	return (
		...
		>
			...
			<div className="flex justify-end">
				...
				>
					{ubahPassword.isPending ? t('Menyimpan…') : t('Simpan Password Baru')}
				</Button>
			</div>
		</form>
	);
...
export default AkunPasswordPanel;
